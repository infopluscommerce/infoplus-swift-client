...
public class SlaSetupAPI: APIBase {
    ...
    /**
     ...
     - parameter slaSetupId: (path) Id of the slaSetup to add a file to 
     - parameter fileName: (path) Name of file 
     - parameter completion: completion handler to receive the data and the error objects
     */
    public class func addSlaSetupFile(slaSetupId slaSetupId: Int32, fileName: String, completion: ((error: ErrorType?) -> Void)) {
        addSlaSetupFileWithRequestBuilder(slaSetupId: slaSetupId, fileName: fileName).execute { (response, error) -> Void in
            completion(error: error);
        }
    }
    ...
    /**
     Attach a file to a slaSetup
     - POST /beta/slaSetup/{slaSetupId}/file/{fileName}
     ...
     - returns: RequestBuilder<Void> 
     */
    public class func addSlaSetupFileWithRequestBuilder(slaSetupId slaSetupId: Int32, fileName: String) -> RequestBuilder<Void> {
        var path = "/beta/slaSetup/{slaSetupId}/file/{fileName}"
        path = path.stringByReplacingOccurrencesOfString("{slaSetupId}", withString: "\(slaSetupId)", options: .LiteralSearch, range: nil)
